...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!question || !docId) {
      return res
        .status(400)
        .json({ error: "Question and document ID are required" });
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        if (errorText.includes("content")) {
          errorMessage =
            "Document content is too long or contains unsupported content. Try uploading a shorter document or one with simpler formatting.";
        } else if (errorText.includes("token")) {
          errorMessage =
            "Document is too long. Please upload a shorter document (under 10 pages).";
        } else {
          errorMessage =
            "Bad request. The document might be too long or contain complex formatting.";
        }
      } else if (response.status === 403) {
        errorMessage =
          "API key rejected. Please check your Google API key and ensure Gemini API is enabled.";
...
router.post("/test-content", async (req, res) => {
  try {
    ...
    const prompt = `Please summarize this text in one sentence: ${cleanedContent.substring(
      0,
      1000
    )}`;
