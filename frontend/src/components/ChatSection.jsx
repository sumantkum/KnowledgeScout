...
function ChatSection({ uploadedDocId, filename }) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          ...
          if (!useLongEndpoint && data.error && data.error.includes("too long")) {
            throw new Error(`${data.error} Try using the 'Process Long Document' option.`);
          }
          throw new Error(data.error || "Invalid request.");
        } else if (res.status === 403) {
          throw new Error("API key issue. Please check your Google API key configuration.");
        } else if (res.status === 408) {
          throw new Error("Request timeout. Try using a shorter question or the 'Process Long Document' option.");
  ...
  const testDocumentContent = async () => {
    ...
    try {
      ...
      if (data.success) {
        setError(`✅ Document test passed! Content length: ${data.contentLength} characters`);
      } else {
        setError(`❌ Document test failed: ${data.error}. Content length: ${data.contentLength} characters`);
      }
    } catch (err) {
